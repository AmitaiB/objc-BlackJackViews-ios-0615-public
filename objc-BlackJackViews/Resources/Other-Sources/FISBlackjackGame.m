...
-(void)hit {
    ...
    [self tallyHandScore];
        NSLog(@"JUST HIT a %@ of %@. isBusted = %@, isBlackjack = %@", randomCard.rank, randomCard.suit, self.isBusted? @"YES":@"NO", self.isBlackjack? @"YES":@"NO");
}
...
-(void)tallyHandScore {
    ...
    [self checkIfBusted:handScoreTemp];
    [self checkIfBlackjack:handScoreTemp];

    self.handScore = [NSNumber numberWithUnsignedInteger:handScoreTemp];
}

-(void)checkIfBusted:(NSUInteger)score {
    self.isBusted = score > 21;
}

-(void)checkIfBlackjack:(NSUInteger)score {
    self.isBlackjack = score == 21;
}
